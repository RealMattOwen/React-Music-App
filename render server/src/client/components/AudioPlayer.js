...
const SkipButton = ({ hidden, back, onClick }) => (
  ...
  >
    <div className="skip_button_inner">
        {back ? <i className="fas fa-step-backward" /> : <i className="fas fa-step-forward" />}
    </div>
  </div>
);
...
const PlayPauseButton = ({ audioPlayer }) => (
  <div
    ...
    className="play_pause_button audio_button"
    onClick={audioPlayer.togglePause}
  >
    <i className={classNames('far', { 'fa-play-circle': audioPlayer.state.paused, 'fa-pause-circle': !audioPlayer.state.paused })} />
  </div>
);
...
const AudioProgressDisplay = (props) => {
  return (
    <div className="audio_progress_parent_container">
      {props.elapsedTime}
      <div
...
class AudioPlayer extends React.Component {
  ...
  constructor (props) {
    ...
    this.currentTrackIndex = this.props.activeTrackIndex;
  ...
  componentWillReceiveProps (nextProps) {
    ...
    this.togglePause(true);
	  ...
	  this.currentTrackIndex = nextProps.activeTrackIndex;

	  const newTrackUrl = this.props.playlist[this.currentTrackIndex].url;

	  if (this.audio.src.match(/\/music(.*)/g)[0] === newTrackUrl) {
		  this.setState({
  ...
  render () {
    ...
    return (
      ...
      >
        <h3 className="song-title">{displayText}</h3>
        ...
        {this.props.controls.map((controlKeyword, index) => {
            const controlProps =
                controlKeyword === 'progress' || controlKeyword === 'progressdisplay'
                    ? { displayText, elapsedTime, fullTime, progressBarWidth, audioPlayer: this }
                    : { audioPlayer : this };
            const Control = keywordToControlComponent[controlKeyword] || null;
            return controlKeyword === 'progress' || controlKeyword === 'progressdisplay' ? <Control {...controlProps} key={this.controlKeys[index]} /> : null;
        })}
        ...
        <div className="volume-slider-container">
          {this.state.volume > 0 ?
            (
              <button onClick={() => {
                this.setState({ overwrittenVolume: this.state.volume }, () => {
                  this.setState({ volume: 0 }, () => this.audio.volume = this.state.volume);
                });
			  }}>
              ...
              <button onClick={() => {
                this.setState({ volume: this.state.overwrittenVolume || 0 }, () => {
                  this.setState({ overwrittenVolume: null }, () => this.audio.volume = this.state.volume);
                });
              }}>
                <i className="fas fa-volume-off" />
              </button>
            )}
          <RangeSlider className="volume-slider" max={1} min={0} onChange={value => this.setState({ volume: value }, () => this.audio.volume = this.state.volume)} step={0.01} tooltip={false} value={this.state.volume} />
        </div>
      </div>
    );
  }
